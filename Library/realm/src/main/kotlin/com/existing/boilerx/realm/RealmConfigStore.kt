...
class RealmConfigStore {
    companion object {
        ...
        fun  initModule(realmConfig: RealmConfiguration, vararg clsList: Class<*>) {
            clsList.forEach { cls ->
                initModule(realmConfig, cls)
            }
        }

        @Suppress("UNCHECKED_CAST")
        fun initModule(realmConfig: RealmConfiguration, cls: Class<*>) {
            // check if class of the module
            val annotation = cls.annotations
                .filter { it.annotationClass.java.name == RealmModule::class.java.name }
                .firstOrNull()
            ...
            if (annotation != null) {
                ...
                moduleAnnotation.classes
                    .filter { clazz ->
                        clazz.java.interfaces.contains(RealmModel::class.java)
                    }
                    .forEach { clazz ->
                        init(clazz.java as Class<RealmModel>, realmConfig)
                    }
                moduleAnnotation.classes
                    .filter { clazz ->
                        clazz.java.superclass == RealmObject::class.java
                    }
                    .forEach { clazz ->
                        init(clazz.java as Class<RealmObject>, realmConfig)
                    }
            }
        }
