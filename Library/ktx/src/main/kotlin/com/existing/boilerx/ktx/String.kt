...
fun String.toUUID(length: Int = 12): String {
    var uuid = ""
    var tmpString = this.replace("[^A-Za-z0-9 ]".toRegex(), "")
    if (this.length > length) {
        tmpString = this.substring(0, length)
    }
    tmpString.forEachIndexed { index, c ->
...
fun String.isAllLetter(): Boolean = Pattern.matches("[0-9a-z]+", this)
